...
function wrap(A,b,c,d)
    return @allocated solve_equations(A,b,c,d)
end

#Wykres alokowanej pamięci w zależności od rozmiaru n macierzy, l = 5, 100 <= n <= 10000
...
for i in 1:length(x_axis)
    ...
    for j in 1:samples
        blockmat(x_axis[i],5,100.0,"out.txt")
        A,b = readA_and_generate_b("./out.txt")
        sleep = wrap(A,b,false,false)
        #print(sleep)
        time_pivot[j] = wrap(A,b,true,false)
        time_no_pivot[j] = wrap(A,b,false,false)
        Ltime_pivot[j] = wrap(A,b,true,true)
        Ltime_no_pivot[j] = wrap(A,b,false,true)
...
ylabel!("Mediana alokowanej pamięci [B]")
savefig("./plotting/l5memory3.png")
...
plot!(legend=:topleft)
xlabel!("Rozmiar macierzy")
ylabel!("Średnia alokowana pamięć [B]")
savefig("./plotting/l5memory4.png")
