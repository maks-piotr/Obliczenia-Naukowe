...
#Wykres błędu wzlgędnego w zależności od rozmiaru n macierzy, l = 5, 100 <= n <= 10000
...
for i in 1:length(x_axis)
    ...
    for j in 1:samples
        blockmat(x_axis[i],5,100.0,"out.txt")
        A,b = readA_and_generate_b("./out.txt")
        time_pivot[j] = norm(solve_equations(A,b,true,false) - x, 2)/norm(x, 2)
        time_no_pivot[j] = norm(solve_equations(A,b,false,false) - x, 2)/norm(x, 2)
    end
...
ylabel!("Mediana błędu względnego x")
savefig("./plotting/l5wynik7.png")

plot(x_axis,y_axis_pivot, label="Częściowy wybór el. głónego", lc=:red, dpi=300)


plot!(legend=:topleft)
xlabel!("Rozmiar macierzy")
ylabel!("Mediana błędu względnego x")
savefig("./plotting/l5wynik8.png")

plot(x_axis,y_axis_no_pivot_s, label="Bez wyboru el. głównego", dpi=300)

plot!(legend=:topleft)
xlabel!("Rozmiar macierzy")
ylabel!("Średni błąd względny x")
savefig("./plotting/l5wynik9.png")


plot(x_axis,y_axis_pivot_s, label="Częściowy wybór el. głónego", lc=:red, dpi=300)
plot!(legend=:topleft)
xlabel!("Rozmiar macierzy")
ylabel!("Średni błąd względny x")
savefig("./plotting/l5wynik10.png")
